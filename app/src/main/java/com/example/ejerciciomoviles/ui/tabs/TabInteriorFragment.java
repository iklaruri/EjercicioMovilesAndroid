...
public class TabInteriorFragment extends Fragment
{
    ...
    public static TabInteriorFragment newInstance()
    {
        return new TabInteriorFragment();
    }
                             ...
                             @Nullable Bundle savedInstanceState)
    {
        ...
        final List<Curso> interiores = new ArrayList<Curso>();

        for( int i = 0; i < ((MainActivity) miContext).getCursos().size(); i++)
        {
            if(((MainActivity) miContext).getCursos().get(i).getTipo().equals("I"))
            {
                interiores.add(((MainActivity) miContext).getCursos().get(i));
            }
        }

        // specify an adapter (see also next example)
        rvAdapter = new RecyclerViewAdapter(interiores,miContext);
